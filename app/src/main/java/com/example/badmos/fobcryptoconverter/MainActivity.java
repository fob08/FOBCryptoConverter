package com.example.badmos.fobcryptoconverter;
...
import com.example.badmos.fobcryptoconverter.Utilities.NetworkUtils;
import com.example.badmos.fobcryptoconverter.data.CurrencyContract;
...
public class MainActivity extends AppCompatActivity implements CryptoAdapter.ListItemClickListener{
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.refresh) {
            ...
            values.put(CurrencyContract.CurrencyEntry.PRICE_RATE,"");
            values.put(CurrencyContract.CurrencyEntry.VOLUME, "");
            values.put(CurrencyContract.CurrencyEntry.LAST_MARKET, "");
            values.put(CurrencyContract.CurrencyEntry.LAST_UPDATE, "");
            getContentResolver().update(CurrencyContract.CurrencyEntry.CONTENT_URI, values,
                    null,
                    null);
            cards.clear();
            cryptoAdapter.updateAdapter(cards);
    ...
    public void createContentValues(String... params) {
        ...
        ContentValues values = new ContentValues();
        values.put(CurrencyContract.CurrencyEntry.CRY_CURRENCY, params[0]);
        values.put(CurrencyContract.CurrencyEntry.CURRENCY, params[1]);
        values.put(CurrencyContract.CurrencyEntry.PRICE_RATE, params[2]);
        values.put(CurrencyContract.CurrencyEntry.VOLUME, params[3]);
        values.put(CurrencyContract.CurrencyEntry.LAST_MARKET, params[4]);
        values.put(CurrencyContract.CurrencyEntry.LAST_UPDATE, params[5]);
        getContentResolver().insert(CurrencyContract.CurrencyEntry.CONTENT_URI, values);
    }
    ...
    public class fetchFromDataBase extends AsyncTask<String[],Void,Cursor>{
        ...
        protected void onPostExecute(Cursor cursor) {
            ...
            while (!cursor.isAfterLast()) {
                if (!cursor.getString(cursor.getColumnIndex(CurrencyContract.CurrencyEntry.PRICE_RATE)).equals("")) {
                    JSONObject pob = new JSONObject();
                    try {
                        pob.put("cryptocurrency", cursor.getString(cursor.getColumnIndex(CurrencyContract.CurrencyEntry.CRY_CURRENCY)));
                        pob.put("currency", cursor.getString(cursor.getColumnIndex(CurrencyContract.CurrencyEntry.CURRENCY)));
                        pob.put("price_rate", cursor.getString(cursor.getColumnIndex(CurrencyContract.CurrencyEntry.PRICE_RATE)));
                        pob.put("volume", cursor.getString(cursor.getColumnIndex(CurrencyContract.CurrencyEntry.VOLUME)));
                        pob.put("last_market", cursor.getString(cursor.getColumnIndex(CurrencyContract.CurrencyEntry.LAST_MARKET)));
                        pob.put("last_update", cursor.getString(cursor.getColumnIndex(CurrencyContract.CurrencyEntry.LAST_UPDATE)));
                    }catch (JSONException e){
                    }
                    cards.add(pob);
                    cryptoAdapter.updateAdapter(cards);
    ...
    public void getData() throws JSONException {
        String[] projection = {CurrencyContract.CurrencyEntry.CRY_CURRENCY,
                CurrencyContract.CurrencyEntry.CURRENCY,
                CurrencyContract.CurrencyEntry.VOLUME,
                CurrencyContract.CurrencyEntry.PRICE_RATE,
                CurrencyContract.CurrencyEntry.LAST_UPDATE,
                CurrencyContract.CurrencyEntry.LAST_MARKET};
